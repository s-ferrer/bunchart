...
describe('Users endpoints', () => {
  ...
  it('user should be able to like an Artwork', async () => {
    ...
    await request(app).post(`/api/users/${userWithArtwork._id}/adds`).send({ artworkId: artwork._id })
    ...
    await request(app).post(`/api/users/${createdLikerUser._id}/likes`).send({ artworkId: artwork._id })

    const finalArtworkUser = (await request(app).get(`/api/users/${userWithArtwork._id}/json`)).body
